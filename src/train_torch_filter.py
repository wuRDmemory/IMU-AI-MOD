...
def compute_delta_p(Rot, p):
    ...
    for k in range(0, k_max):
        ...
        delta_p = Rot[idxs_0].transpose(-1, -2).matmul(
            (p[idxs_end] - p[idxs_0]).unsqueeze(-1).float()).squeeze()
        list_rpe[2] = delta_p
    return list_rpe
...
def prepare_loss_data(args, dataset):
    ...
    if os.path.exists(file_delta_p) and os.path.isfile(file_delta_p):
...
def get_start_and_end(seq_dim, u):
    if seq_dim is None:
        ...
        print((u.shape[0] - seq_dim)/10, u.shape, seq_dim)
...
def precompute_lost(Rot, p, list_rpe, N0):
    ...
    r1 = delta_p.float() / distance
    r2 = delta_p_gt / distance
    return r1 , r2
